...
// TestTextProtocolHTTPMatrix exercises registered routes against actual local HTTP upstreams.
func TestTextProtocolHTTPMatrix(t *testing.T) {
	...
	requests := []string{
		`{"model":"MODEL_X","messages":[{"role":"user","content":"hello"}]}`,
		`{"model":"MODEL_X","input":"hello"}`,
		`{"model":"MODEL_X","max_tokens":64,"messages":[{"role":"user","content":[{"type":"text","text":"hello"}]}]}`,
		`{"model":"MODEL_X","contents":[{"role":"user","parts":[{"text":"hello"}]}]}`,
	}
	responses := []string{
