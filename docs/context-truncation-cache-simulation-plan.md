...
2026-09-09 精简：全局/渠道自定义均只配置窗口与输出预留。阈值固定 90%、保留最近一个完整轮次、安全余量自动计算；旧 JSON 中这三个字段直接忽略，不校验、不恢复、不迁移。总开关关闭覆盖所有渠道，渠道默认继承全局。

| 字段 | 默认/校验 | 含义 |
|---|---|---|
| `window_tokens` | 必填正整数，无跨模型通用默认值；上限 2147483647 | 管理员填写模型上下文窗口 |
| `output_reserve_tokens` | 必填正整数，不超过 1073741823，且与自动安全余量之和小于窗口 | 无可确认输出上限时，管理员提供本地预算预留，不改写输出参数 |
...
```text
S = min(ceil(window_tokens × 2 / 100), 8192)
输入预算 L = min(floor(window_tokens × 90 / 100), window_tokens - R - S)
```
...
- 全局 system/developer 和工具定义独立保护；最近一个完整轮次及其工具依赖闭包固定保留。
...
## 15. 完整上下文预算计数（2026-09-08）

22:37 的 Kimi 请求命中全局规则，但旧估算为 116009、转换后136264，均低于预算150489，故未删轮。
原始 Chat 计数遗漏工具调用参数和历史推理；Claude 转换后仍遗漏 thinking。复用启发式计费估算作为硬预算不充分。

### 最小修复与解耦
- 新增 `relay/contexttruncate/estimate.go`：对协议请求的完整 JSON 文本做 CL100K 本地估算；工具参数、结果、推理、工具定义和输出 schema 均参与。
- 仅在协议确定的图片位置去除 URL/base64 运输内容，图片成本沿用既有估算，避免把图片当作长字符串计费或计入文本预算。工具参数中的同名字段不删除。
- 计数分块按 UTF-8 边界进行，避免长代码标识符造成无界 tokenizer 工作量。CL100K 是通用替代估算，不冒充 Kimi/Claude/Gemini 的原生 tokenizer。
- `relay/context_budget.go` 统一预算与旧计数调用，预算取完整估算与原估算较大值；媒体计数过程恢复原上下文计费值，避免覆盖结算回退所读的 prompt_tokens。
- 初始整轮截断与发送前转换后检查使用相同完整计数；不改变预算公式、渠道配置、轮次与工具依赖保护。

### 计费与客户端
- `before/after` 继续表示原计费估算，新增管理员审计 `budget_before/budget_after` 表示完整预算估算。
- 计费仍用原始未截断请求的 `before` 与真实缓存下界，完整预算估算不进入预扣和结算，不额外收费；未截断继续用原始上游用量。
- 实际截断且存在有效 usage 时，客户端输入取 `before`、`budget_before`、真实输入及缓存下界的较大值，避免响应继续遗漏被转发的历史；输出/真实缓存保持原语义。
- 摘要后的新请求重新计算，不延续上一个请求的较大估算。错误响应、缺失 usage、失败退款与缓存模拟规则不变。

### 验收与边界
- 定向覆盖长工具参数、Chat reasoning_content、Claude thinking、Responses function_call、Gemini functionCall 与 schema；覆盖图片运输字段不计入文本、工具参数相似结构不误删。
- 复现旧估算低于预算但完整历史超限时删旧轮；保护轮仍超限返回本地错误；最终转换增加 thinking 时发送前阻断。
- 验证原计费输入不随预算估算上涨、原始 usage 不变、客户端补全独立，以及四协议 HTTP/SSE 转换与失败退款回归。
- 不识别特定供应商错误文案，不加报错后再次截断，不改自动禁用或恢复历史 Key。
- 图片成本仍是估算；上游可能额外添加上下文或使用不同 tokenizer，本地通过不保证上游接受。真实长请求验收与短请求健康探测分开记录。

### 真实回放发现的关联最小修复
首个候选真实回放已执行删轮，但上游报告工具响应缺失。核对前后结构发现：对非空 DTO 直接 Unmarshal 短消息数组会复用旧数组元素，省略的 tool_calls/reasoning/缓存解析字段残留并落到后续消息，造成重复调用和悬空引用。
计数副本与实际应用截断均改为新建零值 DTO 完整解析成功后替换；不修改轮次分组算法，也不通过删除合法工具调用来掩盖问题。新增多协议旧字段清除和真实残留场景回归。失败候选不切流，修正后重新构建、验收。


## 2026-09-09：两参数配置与输出限幅兼容

- 全局和渠道共用两参数 UI：上下文窗口、输出预留。总开关为正向启用，关闭覆盖所有渠道；渠道默认继承，只有自定义显示两个数字输入。
- 删除 threshold_percent、keep_recent_turns、safety_tokens 的 DTO 与表单字段。旧 JSON 任意值直接忽略，固定阈值 90%、最近一完整轮、自动 2%（上限 8192）安全余量，无迁移层。
- 输出预留为必填正整数，受现有请求输出上界约束；与自动安全余量之和必须小于窗口。它只预留预算，不向上游写入输出上限。
- 精确输出字段的有界正整数 set/直接赋值允许与截断并存；消息、工具、模型、整体配置对象，以及输出字段删除/复制/通配符等操作仍按冲突处理。
- 覆盖顺序不变、不预执行、不追加第二次裁剪。发送前按最终输出重新校验预算与上界，失败保持上游零调用。原始请求较大的输出上限仍可能导致保守裁剪或提前预算不足。
- 请求计费、缓存模拟、客户端 usage、数据库结构和生产渠道参数保持原逻辑。
