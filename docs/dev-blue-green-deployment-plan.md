...
脚本还复核本次候选门禁、`bounded.result` 为 `hold`、公网浏览器完成标记 `public-browser.exit=0`
且无 `.pending` 文件、协议待判定，以及未归因 HTTP 5xx 为零。执行端须在真实公网浏览器
验证成功后先上传非空的 `public-browser.exit.pending`，再原子改名为 `public-browser.exit`。
主动探测失败也要单独审查并记录，不因样本被排除于自然业务比较之外而自动放行。
