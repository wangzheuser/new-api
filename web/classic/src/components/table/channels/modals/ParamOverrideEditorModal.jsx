...
const OPERATION_PHASE_OPTIONS = [
  { label: '上游请求前', value: 'request' },
  { label: '客户端响应前（非流式）', value: 'response' },
  { label: '最终错误响应前', value: 'final_error' },
];

const CONDITION_SOURCE_OPTIONS = [
  { label: '自动（兼容旧版查找）', value: 'auto' },
  { label: '响应 / 请求正文', value: 'body' },
  { label: '语义响应', value: 'semantic' },
  { label: '执行上下文', value: 'context' },
];
...
const parseReturnErrorDraft = (valueText, phase = 'request') => {
...
const buildReturnErrorValueText = (draft = {}, phase = 'request') => {
  const policy = getParamOverrideReturnErrorPolicy(phase);
  const message = String(draft.message || '').trim();
  if (draft.simpleMode) {
    return buildParamOverrideSimpleReturnErrorValueText(message, '');
  }
...
const normalizeOperation = (operation = {}) => ({
  ...
  value_text:
    operation.mode === 'return_error' && typeof operation.value === 'string'
      ? buildParamOverrideSimpleReturnErrorValueText(operation.value, '')
...
const parseInitialState = (rawValue) => {
  ...
  if (document.kind === 'unsupported') {
    return {
      ...
      operations: [createDefaultOperation()],
      jsonText: text,
      jsonError: '当前配置包含可视化编辑器暂不支持的字段，已保留原始 JSON。',
    };
  }
...
const validateOperations = (operations, t) => {
  ...
  if (conditionIssue) {
    return t('第 {{line}} 条操作的第 {{condition}} 个条件缺少路径', {
    ...
    if (mode === 'return_error') {
      ...
      try {
        ...
        if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
          ...
          const statusRaw = parsed.status_code ?? parsed.status;
          if (statusRaw !== undefined) {
            ...
            if (
              ...
              return t('第 {{line}} 条操作的状态码无效', { line });
            }
          }
          if (op.phase === 'response' && parsed.skip_retry === false) {
            return t('响应阶段始终停止内部重试');
          }
        }
      } catch (error) {
...
const ParamOverrideEditorModal = ({ visible, value, onSave, onCancel }) => {
  ...
  useEffect(() => {
    ...
    setJsonError(nextState.jsonError ? t(nextState.jsonError) : '');
    setPreserveRawJson(
      classifyParamOverrideEditorDocument(value).kind === 'unsupported',
    );
    setOperationSearch('');
  ...
  const operationCount = useMemo(
    () =>
      operations.filter((item) => !isParamOverrideOperationBlank(item)).length,
    [operations],
  );
  ...
  const buildOperationsJson = useCallback(
    (sourceOperations, options = {}) => {
      ...
      const filteredOps = sourceOperations.filter(
        (item) => !isParamOverrideOperationBlank(item),
      );
      ...
      const payloadOps = filteredOps.map((operation) => {
        ...
        const payload = { mode };
        if (operation.wire_id) {
          payload.id = operation.wire_id;
        }
  ...
  const switchToVisualMode = () => {
    ...
    if (document.kind === 'unsupported') {
      const message = t(
        '当前配置包含可视化编辑器暂不支持的字段，已保留原始 JSON。',
      );
  ...
  const appendOperationsTemplate = (operationsPayload) => {
    ...
    const existing =
      visualMode === 'operations'
        ? operations.filter((item) => !isParamOverrideOperationBlank(item))
        : [];
  ...
  const updateReturnErrorDraft = (operationId, draftPatch = {}) => {
    ...
    if (!current) return;
    const policy = getParamOverrideReturnErrorPolicy(current.phase);
    const draft = parseReturnErrorDraft(current.value_text, current.phase);
    const nextDraft = {
      ...draft,
      ...draftPatch,
    ...
    updateOperation(operationId, {
      value_text: buildReturnErrorValueText(nextDraft, current.phase),
    });
  };
                      ...
                      {selectedOperation ? (
                        (() => {
                          ...
                          const availableOperationModes =
                            selectedOperation.phase === 'request'
                            ...
                            >
                              ...
                              <Row gutter={12}>
                                <Col xs={24} md={8}>
                                  ...
                                  <Select
                                    value={selectedOperation.phase}
                                    optionList={OPERATION_PHASE_OPTIONS.map(
                                      (option) => ({
                                        ...option,
                                ...
                                <Col xs={24} md={8}>
                                  ...
                                  <Select
                                    value={mode}
                                    optionList={availableOperationModes}
                                    onChange={(nextMode) => {
                                      updateOperation(selectedOperation.id, {
                                        mode: nextMode,
                                        value_text:
                                          nextMode === 'return_error' &&
                                          mode !== 'return_error'
                                            ? buildParamOverrideSimpleReturnErrorValueText(
                                                selectedOperation.value_text,
                                              )
                                            ...
                                            <Row gutter={12}>
                                              <Col xs={24} md={6}>
                                                ...
                                                <Select
                                                  ...
                                                  optionList={CONDITION_SOURCE_OPTIONS.filter(
                                                    (option) =>
                                                      option.value !==
                                                        'semantic' ||
                                                      selectedOperation.phase ===
