...
const channelAutoDisableRows = [
  [
    'Automatically protect channels after qualifying failures.',
    '在符合规则的失败发生后自动保护渠道。',
    '在符合規則的失敗發生後自動保護渠道。',
    'Protège automatiquement les canaux après des échecs répondant aux critères.',
    '条件に該当する失敗後にチャネルを自動保護します。',
    'Автоматически защищает каналы после ошибок, соответствующих условиям.',
    'Tự động bảo vệ kênh sau các lỗi đáp ứng điều kiện.',
  ],
  [
    'Automatic disable protection',
    '自动禁用保护',
    '自動停用保護',
    'Protection de désactivation automatique',
    '自動無効化保護',
  ...
  [
    ...
    'Redis 未启用，统计型临时自动禁用不会运行。',
    'Redis 未啟用，統計型暫時自動停用不會執行。',
    'Redis n’est pas activé. La désactivation temporaire statistique ne fonctionnera pas.',
    'Redis が有効ではないため、統計ベースの一時自動無効化は動作しません。',
    'Redis не включён. Статистическое временное автоотключение работать не будет.',
    'Redis chưa được bật. Tính năng tạm tự động tắt theo thống kê sẽ không hoạt động.',
  ],
  [
    'Immediate auto-disable status codes',
    '立即自动禁用状态码',
    '立即自動停用狀態碼',
    'Codes d’état de désactivation automatique immédiate',
    '即時自動無効化ステータスコード',
    'Коды состояния для немедленного автоотключения',
    'Mã trạng thái tự động tắt ngay',
  ],
  [
    'Error-rate status codes',
    '错误率统计状态码',
    '錯誤率統計狀態碼',
    'Codes d’état comptabilisés dans le taux d’erreur',
    'エラー率集計ステータスコード',
    'Коды состояния для расчёта доли ошибок',
    'Mã trạng thái tính tỷ lệ lỗi',
  ],
  [
    'Only real upstream HTTP responses in these ranges count as statistical failures.',
    '仅真实上游 HTTP 响应中处于这些范围的状态码会计为统计失败。',
    '僅真實上游 HTTP 回應中位於這些範圍的狀態碼會計為統計失敗。',
    'Seules les réponses HTTP réelles du fournisseur comprises dans ces plages sont comptées comme échecs.',
    'これらの範囲に該当する実際の上流 HTTP 応答だけが統計上の失敗として記録されます。',
    'Статистическими ошибками считаются только реальные HTTP-ответы вышестоящего сервиса из этих диапазонов.',
    'Chỉ phản hồi HTTP thực tế từ thượng nguồn trong các khoảng này được tính là lỗi thống kê.',
  ],
  [
    'Statistics window (minutes)',
    '统计周期（分钟）',
    '統計週期（分鐘）',
    'Fenêtre statistique (minutes)',
    '集計期間（分）',
    'Окно статистики (минуты)',
    'Khoảng thời gian thống kê (phút)',
  ],
  [
    'Count upstream responses from the latest time window.',
    '统计最近时间窗口内的上游响应。',
    '統計最近時間視窗內的上游回應。',
    'Compte les réponses du fournisseur dans la dernière fenêtre temporelle.',
    '直近の時間枠にある上流応答を集計します。',
    'Учитывает ответы вышестоящего сервиса за последнее окно времени.',
    'Đếm phản hồi thượng nguồn trong khoảng thời gian gần nhất.',
  ],
  [
    'Minimum upstream responses',
    '最小上游响应数',
    '最小上游回應數',
    'Nombre minimal de réponses du fournisseur',
    '上流応答の最小件数',
    'Минимальное число ответов вышестоящего сервиса',
    'Số phản hồi thượng nguồn tối thiểu',
  ],
  [
    'Do not disable a channel before this sample size is reached.',
    '样本数达到该值之前不会禁用渠道。',
    '樣本數達到此值之前不會停用渠道。',
    'Le canal ne sera pas désactivé avant que cet échantillon soit atteint.',
    'このサンプル数に達するまでチャネルは無効化されません。',
    'Канал не отключается, пока не будет достигнут этот размер выборки.',
    'Không tắt kênh trước khi đạt kích thước mẫu này.',
  ],
  [
  ...
  [
    ...
    '错误率达到该值时临时禁用渠道。',
    '錯誤率達到此值時暫時停用渠道。',
    'Désactive temporairement le canal lorsque le taux d’erreur atteint cette valeur.',
    'エラー率がこの値に達した場合、チャネルを一時的に無効化します。',
    'Временно отключает канал, когда доля ошибок достигает этого значения.',
    'Tạm thời tắt kênh khi tỷ lệ lỗi đạt giá trị này.',
  ],
  [
    'Temporary disable duration (minutes)',
    '临时禁用时长（分钟）',
    '暫時停用時長（分鐘）',
    'Durée de désactivation temporaire (minutes)',
    '一時無効化時間（分）',
    'Длительность временного отключения (минуты)',
    'Thời gian tạm tắt (phút)',
  ],
  [
    'The channel automatically returns to routing after this duration.',
    '超过该时长后，渠道会自动恢复参与路由。',
    '超過此時長後，渠道會自動恢復參與路由。',
    'Le canal revient automatiquement dans le routage après cette durée.',
    'この時間が経過すると、チャネルは自動的にルーティングへ復帰します。',
    'После этого времени канал автоматически возвращается в маршрутизацию.',
    'Kênh tự động trở lại định tuyến sau khoảng thời gian này.',
  ],
  [
    'Use system auto-disable rules',
    '使用系统自动禁用规则',
    '使用系統自動停用規則',
    'Utiliser les règles système de désactivation automatique',
    'システムの自動無効化ルールを使用',
    'Использовать системные правила автоотключения',
    'Dùng quy tắc tự động tắt của hệ thống',
  ],
  [
    'Disable this option to override the statistical thresholds for this channel.',
    '关闭此选项可单独设置该渠道的统计阈值。',
    '關閉此選項可單獨設定此渠道的統計閾值。',
    'Désactivez cette option pour remplacer les seuils statistiques de ce canal.',
    'このオプションを無効にすると、このチャネルの統計しきい値を上書きできます。',
    'Отключите этот параметр, чтобы переопределить статистические пороги канала.',
    'Tắt tùy chọn này để ghi đè các ngưỡng thống kê cho kênh.',
  ],
  [
  ...
  [
    ...
    '上流応答数：',
    'Ответы вышестоящего сервиса:',
    'Phản hồi thượng nguồn:',
  ],
  [
    'Matching errors:',
    '匹配错误数：',
  ...
  [
    'Release temporary disable',
    '解除临时禁用',
    '解除暫時停用',
    'Lever la désactivation temporaire',
    '一時無効化を解除',
...
for (const [key, zh, zhTW, fr, ja, ru, vi] of channelAutoDisableRows) {
  Object.assign(newKeys.en, { [key]: key })
  Object.assign(newKeys.zh, { [key]: zh })
  Object.assign(newKeys['zh-TW'], { [key]: zhTW })
  Object.assign(newKeys.fr, { [key]: fr })
