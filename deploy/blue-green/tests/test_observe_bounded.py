...
class BoundedObservationTest(unittest.TestCase):
    ...
    def test_inconclusive_does_not_repeat_without_opt_in(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                (root/"state/bounded.result").read_text(),
                "result=evidence_inconclusive observation_exit=3 action=hold\n",
            )
...
''')
                ...
                if first == 3 and second != 1:
                    ...
                    self.assertIn('action=hold', (root/'state/bounded.result').read_text())
                if calls == 2:
